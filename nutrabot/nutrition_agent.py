...
import chromadb
from agents import Agent, WebSearchTool, function_tool, Tool
...
class NutritionAgent:
    ...
    def __init__(self):
        ...
        self.exa_search_mcp: MCPServerStreamableHttp = self.get_exa_search_mcp()

        self.calorie_agent: Agent = self._get_calorie_agent()

        self.calorie_calculator_tool: Tool = self._get_calorie_agent().as_tool(
            tool_name="calorie-calculator",
            tool_description="Use this tool to calculate the calories of a meal and its ingredients",
        )

        self.breakfast_planner_tool: (
            Tool
        ) = self._get_healthy_breakfast_planner_agent().as_tool(
            tool_name="breakfast-planner",
            tool_description="Use this tool to plan a a number of healthy breakfast options",
        )

        self.breakfast_price_checker_agent: Agent = (
            self._get_breakfast_price_checker_agent()
        )

        self.breakfast_advisor: Agent = self._get_breakfast_advisor()

    def get_exa_search_mcp(self):
        exa_search_mcp: MCPServerStreamableHttp = MCPServerStreamableHttp(
            name="Exa Search MCP",
            params={
    ...
    # Agent 1: Calorie agent
    def _get_calorie_agent(self) -> Agent:
        return Agent(
            ...
            * You follow this workflow:
              ...
              1) If you couldn't find the exact match for the food or you need to look up the ingredients, search the EXA web to figure out the exact ingredients of the meal.
                 Even if you have the calories in the web search response, you should still use the calorie_lookup_tool to get the calorie information of the ingredients to make sure the information you provide is consistent.
            ...
            * You only answer questions about food.
            ...
            mcp_servers=[self.exa_search_mcp],
            # input_guardrails=[food_topic_guardrail],
        )

    # Agent 2: Healthy Breakfast Plan Adviser
    def _get_healthy_breakfast_planner_agent(self) -> Agent:
        return Agent(
    ...
    # Agent 3: checking the breakfast's price
    def _get_breakfast_price_checker_agent(self) -> Agent:
        return Agent(
    ...
    # Agent 4: Gluing everything together
    def _get_breakfast_advisor(self) -> Agent:
        return Agent(
            ...
            * Based on the breakfast meals and the calories that you get from upstream agents, create a meal plan for the user. For each meal, give a name, the ingredients, and the calories.
            * You only answer questions about food.
            ...
            3) Handoff the breakfast meals and the calories to the Breakfast Price Checker Assistant to add the prices in the last step.
            """,
            tools=[self.breakfast_planner_tool, self.calorie_calculator_tool],
            handoff_description="""
            Create a concise breakfast recommendation based on the user's preferences. Use Markdown format.
            """,
            handoffs=[self.breakfast_price_checker_agent],
            input_guardrails=[food_topic_guardrail],
        )
